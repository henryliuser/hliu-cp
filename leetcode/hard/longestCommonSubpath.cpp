...
public:
    ...
    int longestCommonSubpath(int n, vector<vector<int>>& paths) {
        PolyHash::base = PolyHash::gen_base(n, PolyHash::mod);
        PolyHash::mxPow = 0;
        PolyHash::pow1 = {1};
        PolyHash::pow2 = {1};
        Ap = &paths;
        ...
        sort(begin(paths), end(paths), [](const auto &a, const auto &b) {
            return a.size() < b.size();
        });
        for (auto& p : paths)
            H.emplace_back(PolyHash(p));

        int lo = 0, hi = paths[0].size();
        while (lo < hi) {
